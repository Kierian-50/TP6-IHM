...
public class DisplayConnexion {
    ...
    public DisplayConnexion(){
        try{
            String path = URLDecoder.decode(DisplayConnexion.class.getProtectionDomain().getCodeSource().getLocation().getPath(),"UTF-8");
            ...
            String placementData = path+data;

            File file = new File(placementData);

            if(!file.exists()){
                RWFile.createFile(placementData);
            }
